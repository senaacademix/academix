...
export type CalendarBodyProps = {
  ...
  onShowMore?: (day: number, features: Feature[]) => void;
};

export const CalendarBody = ({ features, children, onShowMore }: CalendarBodyProps) => {
      ...
      >
        ...
        {featuresForDay.length > 3 && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onShowMore?.(day, featuresForDay);
            }}
            className="block text-muted-foreground text-xs hover:text-primary hover:underline cursor-pointer transition-colors"
          >
